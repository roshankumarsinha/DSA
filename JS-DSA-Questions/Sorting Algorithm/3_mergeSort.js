...
// Intuition:
// 1. Merge Sort is a divide and conquer algorithm, it divides the given array into equal parts and then merges the 2 sorted parts.
...
function merge(arr, left, mid, right) {
  ...
  // We will use the 2-pointers technique to merge the two halves.
  // Whoever has the smaller element will be added to the merged array first, and the respective pointer will be incremented.
  let i = left; // Start of left half
  let j = mid + 1; // Start of right half
  ...
  while (i <= mid) temp.push(arr[i++]); // If left half has remaining elements, add them to temp
  while (j <= right) temp.push(arr[j++]); // If right half has remaining elements, add them to temp
...
function mergeSort(arr, left, right) {
  if (left >= right) return; // Base case: one or zero elements, already sorted

  const mid = left + Math.floor((right - left) / 2); // Find mid
